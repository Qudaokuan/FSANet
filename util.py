...
def calc_ssim(sr, hr, scale):
    ...
    convert = sr.new_tensor(gray_coeffs).cuda().view(1, 3, 1, 1) / 256
    sr = sr.mul(convert).sum(dim=1)[..., shave:-shave, shave:-shave].cpu().numpy()
    hr = hr.mul(convert).sum(dim=1)[..., shave:-shave, shave:-shave].cpu().numpy()
    return compare_ssim(sr,hr,data_range=1,channel_axis=0,gaussian_weights=True) 